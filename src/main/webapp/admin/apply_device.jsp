...
<table class="layui-table" id="tb" style="text-align: center;">
    <thead>
      <tr>
        ...
        <th style="text-align: center;">申请设备号</th>
...
<script type="text/javascript">
	$(function(){
		$.ajax({
			url: "../user/findAllApplyDeviceByID.action",
			type: "post",
			dataType: "json",
			data: {},
			success: function(data){
				...
				for(var i=0;i<data.length;i++){
					...
					inHtml +='<td>'+data[i].device_id+'</td>';
					inHtml +='<td>'+data[i].apply_state+'</td>';
					inHtml +='<td>'+data[i].deal_man+'</td>';
					inHtml +='<td><a href="../user/passApplyDevice.action?&apply_id='+data[i].apply_id+'" id="edit" class="layui-btn layui-btn-big">👌 通过</a><a href="../user/notPassApplyDevice.action?&apply_id='+data[i].apply_id+'" id="del" class="layui-btn layui-btn-danger">⛔ 不通过</a></td></tr>';
				}
				$("#content").html(inHtml);
			},
			error: function(){
